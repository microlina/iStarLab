...
public class DependencyLinkReorientCommand extends EditElementCommand {
	...
	public boolean canExecute() {
		if (false == getElementToEdit() instanceof DependencyLink) {
			return false;
		}
	...
	protected boolean canReorientSource() {
		...
		Node target = getLink().getDependencySecondNode();
		...
		return IstarBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistDependencyLink_4036(container, getLink(),
						getNewSource(), target);
	}
	...
	protected boolean canReorientTarget() {
		...
		Node source = getLink().getDependencyNode();
		if (!(getLink().eContainer() instanceof istar.ISTAR)) {
			return false;
		}
		istar.ISTAR container = (istar.ISTAR) getLink().eContainer();
		return IstarBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistDependencyLink_4036(container, getLink(), source,
						getNewTarget());
	}
	...
	protected CommandResult reorientSource() throws ExecutionException {
		getLink().setDependencyNode(getNewSource());
	...
	protected CommandResult reorientTarget() throws ExecutionException {
		getLink().setDependencySecondNode(getNewTarget());
		return CommandResult.newOKCommandResult(getLink());
	}
	 ...
	 * @generated
	 */
	protected DependencyLink getLink() {
		return (DependencyLink) getElementToEdit();
	}
